...
// 1. REVERT Kepatuhan Tab
const kepatuhanRegex = /<tbody className="text-sm">[\s\S]*?\{scan\.rawData\?\.includes\(':'\)\s*\?\s*scan\.rawData\.split\(':'\)\[1\][\s\S]*?<\/td>[\s\S]*?<\/tr>[\s\S]*?<\/tbody>/;

const originalKepatuhan = `<tbody className="text-sm">
...
if (content.match(kepatuhanRegex)) {
  content = content.replace(kepatuhanRegex, originalKepatuhan);
  console.log("Restored Kepatuhan tab");
} else {
  console.log("Could not find Kepatuhan regex");
}

// 2. PATCH Scanner Tab
const scannerRegex = /<tbody className="text-sm">[\s\S]*?<td className="p-4 font-mono text-xs text-slate-400">08:14:22 WIB<\/td>[\s\S]*?<\/tbody>/;

const newScannerTbody = `<tbody className="text-sm">
...
if (content.match(scannerRegex)) {
  content = content.replace(scannerRegex, newScannerTbody);
  console.log("Patched Scanner tab");
} else {
  console.log("Could not find Scanner regex");
}
