                      ...
                      {mobileScans.length > 0 ? mobileScans.map((scan, i) => (
                        <tr key={scan.id || i} className="border-b border-slate-800/50 hover:bg-slate-800/20 animate-in fade-in slide-in-from-top-2">
                          ...
                          <td className="p-4 font-bold text-slate-300">
                            {scan.rawData?.includes(':') ? scan.rawData.split(':')[2] : (scan.jemaahId ? \`Jemaah ID: \${scan.jemaahId}\` : (scan.rawData || 'Tidak Dikenal'))}
                          </td>
                          <td className="p-4 text-slate-500 font-mono text-xs">
                             {scan.rawData?.includes(':') ? scan.rawData.split(':')[1] : (scan.jemaahId || (scan.rawData || '-'))}
                          </td>
...
if (content.match(regex)) {
  ...
  console.log("Patched Admin Table successfully");
} else {
  console.log("Could not find table body regex");
}
