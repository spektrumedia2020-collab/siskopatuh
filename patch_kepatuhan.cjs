...
if (content.includes(uiStart) && content.includes(uiEnd)) {
  ...
  const after = content.substring(content.indexOf(uiEnd) + 9);
                  ...
                  <table className="w-full text-left text-sm">
                    ...
                    <tbody className="divide-y divide-slate-800/50">
                      ...
                      {filteredDirektoriList.map((item, idx) => {
                        ...
                        // Let's add PT Atlas Tour if it's not in the data to match the image,
                        // actually the image has: Al-Dawood, Gaido, Sultanah, Atlas Tour.
                        // I will rely on the mapped data but format it nicely.
  ...
  // Make sure we have the exact data from the image if it is missing
  // Modify the data mock array
  const mockOld = `data = [
  ...
  const mockNew = `data = [
              ...
              { name: 'PT. Gaido Azza Darussalam Indonesia', type: 'PIHK', status: 'MENUNGGU', id: 'PIHK-003' },
              { name: 'PT. Sultanah Nafisah Mandiri', type: 'PPIU', status: 'MENUNGGU', id: 'PPIU-001' },
              { name: 'PT Atlas Tour', type: 'PPIU', status: 'MENUNGGU', id: 'PPIU-002' },
           ];`;
